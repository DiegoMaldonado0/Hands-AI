  ...
  return (
    <Section className="relative pt-20 pb-10 text-white">
      ...
      <div className="container mx-auto text-center relative z-10">
        ...
        <p className="text-lg mb-8">Test your skills and mimic the letters in sign language. Can you keep up with Simon?</p>
        <div className="flex flex-col md:flex-row items-center justify-center gap-6">
          <div className="w-full md:w-1/2">
            <video id="video" className="w-full rounded-lg border border-gray-300 shadow-lg" autoPlay playsInline muted />
            <button
          ...
          <div className="w-full md:w-1/2 text-center">
            <div className="p-6 bg-white text-black rounded-lg shadow-md mb-4">
              <motion.p className="text-4xl font-bold" key={targetLetter} initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ duration: 0.5 }}>
                Simon Says: {targetLetter}
              </motion.p>
            </div>
            ...
            {gameMessage && <motion.p className="mt-4 text-red-600 font-bold" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>{gameMessage}</motion.p>}
            <button onClick={toggleGame} className="mt-6 px-6 py-3 bg-green-500 hover:bg-green-600 text-white font-bold rounded-lg shadow-md transition">
              {isGameOn ? "Stop the Game" : "Start Playing!"}
            </button>
          </div>
        </div>
      </div>
